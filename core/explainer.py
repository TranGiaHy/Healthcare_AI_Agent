...
class AIExplainer:
    ...
    2. Optionally use FLAN-T5-Large to rewrite that explanation in more natural language.
    3. If model output is weak or fails, safely fall back to the deterministic version.
    """
    ...
    def _build_structured_explanation(self, payload: Dict[str, Any]) -> str:
        ...
        lines.append("")
        lines.append(
            f"Based on the symptoms entered, the system found a health insight that is most closely related to: {diagnosis}."
        )
        lines.append(
